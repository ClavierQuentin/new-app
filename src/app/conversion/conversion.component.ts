...
export class ConversionComponent implements OnInit  {
  ...
  //           .subscribe({
  //             next : (res :number) => { this.montantConverti = res; console.log("resultat obtenu en différé")} ,
  ...
  async onConvertir(){
    try{
      let devise : Devise ;
      for(let i = 0; i < this.listeDevises.length; i++){
        if(this.listeDevises[i].code == this.codeDeviseCible){
          devise = this.listeDevises[i];
        }
      }      
      this.montantConverti =  await firstValueFrom(this._deviseService.convertir$(this.montant, this.codeDeviseSource, this.codeDeviseCible));
    } catch(ex){
      console.log(ex);
    }
  }
  ...
  initListeDevises(tabDevises : Devise[]){
    ...
    if(tabDevises && tabDevises.length > 0){
      ...
      this.codeDeviseCible = tabDevises[1].code; //valeur par défaut
    }
  }
